...
from .user import UserPublicWithTasks, UserPublic
...
async def show_users_and_tasks():
    async with async_session() as session:
        ...
        for user in users:
            public_user = UserPublicWithTasks.model_validate(user)
            print(f"{public_user.email}:")
            for task in public_user.tasks:
                print(f"  - {task.title}")
...
async def show_tasks_and_users():
    async with async_session() as session:
        ...
        for task in tasks:
            public_task = TaskPublicWithUsers.model_validate(task)
            print(f"{public_task.title}:")
            for user in public_task.users:
                print(f"  - {user.email}")
